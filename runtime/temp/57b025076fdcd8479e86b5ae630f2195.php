<?php /*a:1:{s:53:"D:\Timount\mall\application\icon\view\icon\index.html";i:1549848305;}*/ ?>
<!DOCTYPE html>
<html>
<head>
    <!-- 下面是定义标签库 -->
...
<div class="layui-fluid">
    <div class="layui-card">
                ...
                <div class="layui-form-item">
                    <div class="layui-inline">
                        <div style="float: left">
                            <div class="layui-inline">
                                <input class="layui-input" name="key" id="key"  autocomplete="off" placeholder="请输入icon名称" style="width: 400px">
                            </div>
        ...
        <div class="layui-btn-table">
            <!--<a href="#" id="add" class="layui-btn layui-btn-sm layui-btn-normal" style="float:left;"><i class="fa fa-plus" aria-hidden="true"></i>添加icon</a>
...
<script type="text/html" id="sort">
    <input name="{{d.icon_id}}" data-id="{{d.icon_id}}" class="list_order layui-input" value=" {{d.icon_sort}}" size="10"/>
</script>
...
<script>
    ...
    layui.use(['table','form'], function() {
        ...
        tableIn = table.render({
            ...
            height: 'full-170',
            cols: [[
                {field: 'icon_title', title: 'icon名称',align: 'center', width: 160,templet: '#media_balance'},
                {field: 'icon_img', title: 'icon缩略图',align: 'center', width: 200,templet: '#index'},
        ...
        table.on('tool(list)', function(obj) {
            var data = obj.data;
            if (obj.event === 'del'){
                layer.confirm('您确定要删除该icon吗？', function(index){
                    var loading = layer.load(1, {shade: [0.1, '#fff']});
                    $.post("<?php echo url('del'); ?>",{id:data.icon_id},function(res){
                        layer.close(loading);
                        if(res.code===1){
                ...
                layer.open({
                    type: 2,
                    title: '新增icon',
                    content: '<?php echo url("edit"); ?>?id='+ data.icon_id,
        ...
        $('body').on('blur','.list_order',function() {
            ...
            var sort = $(this).val();
            var loading = layer.load(1, {shade: [0.1, '#fff']});
            $.post('<?php echo url("sort"); ?>',{icon_id:icon_id,sort:sort},function(res){
                layer.close(loading);
                if(res.code === 1){
                    layer.msg(res.msg, {time: 1000, icon: 1});
                    tableIn.reload();
